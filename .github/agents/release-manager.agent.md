...
Release notes are mandatory for every release, including patch releases. They must be reviewed for accuracy before publication; never publish autogenerated notes as the final release body.
...
Read the actual commits and code changes (not just commit messages) since the previous tag. Group changes by user impact and include only meaningful, user-facing changes. Mention notable fixes, security remediations, and dependency changes when they affect users or operational risk. Do not invent details or include internal-only churn.
