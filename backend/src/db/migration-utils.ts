...
export async function runAlterMigrations(client: Client): Promise<{ success: boolean; errors: string[] }> {
	...
	const createTableMigrations = [
      ...
      refill_date INTEGER NOT NULL DEFAULT (strftime('%s','now'))
		...
		`CREATE TABLE IF NOT EXISTS as_needed_intake_events (
			id INTEGER PRIMARY KEY AUTOINCREMENT,
			...
			quantity_unit TEXT NOT NULL CHECK(quantity_unit IN ('pills', 'ml', 'puffs', 'injections', 'application')),
			person_name TEXT NOT NULL DEFAULT '',
			source TEXT NOT NULL DEFAULT 'owner_as_needed',
			status TEXT NOT NULL DEFAULT 'active' CHECK(status IN ('active', 'reversed')),
			stock_effect_milli INTEGER NOT NULL DEFAULT 0 CHECK(stock_effect_milli >= 0),
			stock_effect_reason TEXT NOT NULL DEFAULT 'applied' CHECK(stock_effect_reason IN ('applied', 'non_measurable', 'before_correction', 'superseded_by_correction')),
			stock_cutoff_at INTEGER NOT NULL DEFAULT 0,
			replaces_event_id INTEGER REFERENCES as_needed_intake_events(id),
			reversed_at INTEGER,
			reversal_idempotency_key_hash TEXT,
	...
	const createIndexMigrations = [
		...
		`CREATE UNIQUE INDEX IF NOT EXISTS as_needed_intake_events_event_id_unique ON as_needed_intake_events(event_id)`,
		`CREATE UNIQUE INDEX IF NOT EXISTS as_needed_intake_events_anchor_unique ON as_needed_intake_events(dose_tracking_id)`,
		`CREATE UNIQUE INDEX IF NOT EXISTS as_needed_intake_events_owner_key_unique ON as_needed_intake_events(user_id, idempotency_key_hash)`,
		`CREATE UNIQUE INDEX IF NOT EXISTS as_needed_intake_events_owner_reversal_key_unique ON as_needed_intake_events(user_id, reversal_idempotency_key_hash) WHERE reversal_idempotency_key_hash IS NOT NULL`,
		`CREATE UNIQUE INDEX IF NOT EXISTS as_needed_intake_events_replacement_unique ON as_needed_intake_events(replaces_event_id) WHERE replaces_event_id IS NOT NULL`,
		`CREATE INDEX IF NOT EXISTS as_needed_intake_events_medication_occurred_idx ON as_needed_intake_events(user_id, medication_id, occurred_at)`,
		`CREATE INDEX IF NOT EXISTS as_needed_intake_events_owner_status_occurred_idx ON as_needed_intake_events(user_id, status, occurred_at)`,
