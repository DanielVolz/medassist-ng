...
const medicationBodyOpenApiSchema = {
	...
	properties: {
		...
		pillForm: { type: "string", enum: ["capsule", "tablet"] },
		...
		totalPills: { type: "integer", minimum: 1 },
		looseTablets: { type: "integer", minimum: 0 },
		pillWeightMg: { type: "number", minimum: 0 },
		doseUnit: { type: "string", enum: ["mg", "g", "mcg", "ml", "IU", "units", "drops", "puffs"] },
		medicationStartDate: { type: "string", pattern: "^\\d{4}-\\d{2}-\\d{2}$" },
		medicationEndDate: { type: "string", pattern: "^\\d{4}-\\d{2}-\\d{2}$" },
		autoMarkObsoleteAfterEndDate: { type: "boolean" },
		expiryDate: { type: "string" },
		notes: { type: "string", maxLength: 2000 },
		prescriptionEnabled: { type: "boolean" },
		prescriptionAuthorizedRefills: { type: "integer", minimum: 0 },
		prescriptionRemainingRefills: { type: "integer", minimum: 0 },
